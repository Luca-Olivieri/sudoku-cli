...
impl game::Board {
    ...
    pub fn draw_grid(
        ...
        for y in 0..N {
            ...
            for x in 0..N {
                let coords = game::Coordinates{row: y, col: x};
                let val = self.get_cell(&coords);
                let ch = if val == game::UNSET_CELL { ' ' } else { char::from_digit(val as u32, 10).unwrap() };
